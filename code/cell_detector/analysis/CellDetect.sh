...
# cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/detection/20180109_SCCNNdetection/parameters-detection.txt "${1}"
FULLDIRNAMEESC=$(echo $FULLDIRNAME | sed 's_/_\\/_g') 
for s in $SAMPLES; do	
  echo "Create bsub for ${s}"
  BNAME=`basename ${s}`
  cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/detection/20180109_SCCNNdetection/header_cell_detection "${FULLDIRNAME}/bsubHEdetection/${BNAME}_extract.bsubHEdetection"  
